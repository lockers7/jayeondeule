...
| 2026-05-25 | Claude | **Phase 4 C+A+B 완료** (commit `e09aea6` `31b4041` `676e4ae` `6472406`). C: question_analyzer fallback 키워드 분기 (Ollama 503 시 web_search 대신 농장 도구). A: `agent_one_shot` 도구 — 채팅에서 1회 ReAct 분석. B: `agent_subscriptions` + `agent_user_alerts` (migration 007), agent_scheduler 60s polling, `tools_agent_sub` 4도구. ChromaDB `chat_analyzer_raw` 갱신 (8269자, 신규 4 도구 라우팅 가이드). hotfix 4건 — 1) `get_thresholds` 최신 row, 2) `set_relay` semantic pin_map 정렬, 3) fallback web_search→농장, 4) fallback 등록/조회/취소/알림 세분화. pytest 143 PASS. |
| 2026-05-25 | Claude | **vLLM 호환 추상화 레이어** (`docs/dev_vllm_migration.md`). 신규 `agri_ai_core/src/ai/llm_backend_vllm.py` (OpenAI 호환, options/format/response 매핑). `_ollama_chat` + `ai_control._call_llm` + `ai_monitor_agent._call_llm` 에 `LLM_BACKEND` env switch (default ollama 유지). `setup/vllm_install.sh` + systemd unit + agriAiCore 18번. 22 unit + 165 회귀 PASS. **운영 영향 0** (vLLM 미활성 default). |
| 2026-05-25 | Claude | **Phase 3 완료**. write 도구 4종(`set_relay/set_threshold/set_growth_stage/send_user_alert`) + `agent_pending_actions` 30초 취소 큐 + `agent_pending_worker` polling 데몬. safety_guard (daily_limit/cooldown), 인자 화이트리스트, 인터록 게이트 위임. pytest 94 PASS (read 17 + monitor 27 + scheduler 15 + write 22 + worker 13). 종단간 검증: alert 즉시(0.2s), set_relay/set_threshold 30초 후 실행(존재 안 하는 호기 → safe failed), 사용자 취소(id=12) worker 무시 — 4개 시나리오 모두 의도대로. systemd `agent_pending_worker.service` + `agriAiCore` 17번. `prompt_block_m.AGENT_MONITOR_SYSTEM` 1944자로 갱신 (write 정책 + 예시 2종 추가). |
