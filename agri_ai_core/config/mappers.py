...
_RELAY_DESCRIPTIONS = {
    'water_heater_flag': (
        ...
        '【🔴 절대 금지 — 실손 3중 피해 룰 (사용자 정의·2026-05-31)】 '
        'water_heater_flag = ON + drainage_motor_flag = ON 동시 가동은 절대 금지. 발생 시 다음 3가지 피해: '
        '(a) 효과 0: 새 차가운 지하수가 끊임없이 유입되므로 탱크 수온이 절대 오르지 않음 → 가열 목적 자체가 무효. '
        '(b) 전기료 폭증: 히터가 목표 수온에 영원히 도달하지 못해 무한 가동 → 전력 소비 폭증, 전기료 막대한 손해. '
        '(c) 히터 장비 고장: 가열 코일·서모스탯이 끊임 없이 가열 상태 유지 → 과열 누적 → 코일 단선·소자 소손으로 히터 자체 파손. '
        '→ 두 플래그는 반드시 반대값으로만 결정. 가온 필요 시 (heater=true, drainage=false), 비가온 시 (heater=false, drainage=true). '
    ...
    'drainage_motor_flag': (
        ...
        '【🔴 절대 금지 — 실손 3중 피해 룰 (사용자 정의·2026-05-31)】 '
        'drainage_motor_flag = ON + water_heater_flag = ON 동시 가동은 절대 금지. '
        '(a) 가열 효과 0 (새 차가운 지하수 계속 유입), '
        '(b) 히터 무한 가동 → 전기료 폭증, '
        '(c) 가열 코일 과열 누적 → 히터 장비 고장. '
        '두 플래그는 반드시 반대값. '
        '【용도 시나리오】 '
        ...
        '【LLM 제어 권한 (2026-05-01 변경, 2026-05-31 PROTECTED 해제)】 LLM 이 반드시 매 사이클 함께 결정. '
        '수온히터·배수밸브 상호배타 룰을 LLM 이 직접 적용한다. '
        '· 가온 필요 (수온 < 적정하한): water_heater_flag=true, drainage_motor_flag=false. '
        '· 비가온/수온 정상·상승: water_heater_flag=false, drainage_motor_flag=true. '
        '· 두 플래그가 같은 값 (둘 다 true / 둘 다 false) 인 응답은 룰 위반 — 시스템 거부 대상. '
    ...
    # 미사용 릴레이(E타입 2호 relay_3·relay_4, 1·3호 relay_9/15)는 매핑에 없음 —
    # 핀맵·라벨·별칭·LLM 프롬프트에서 자동 누락되며, 백엔드의
    # force_off_unmapped_relays() 가 어떤 모드에서도 ON 을 차단함.
}

# PROTECTED — LLM 자율 변경 금지 (별도 스케줄·환경 로직이 관리)
# drainage_motor_flag 는 포함 금지 — PROTECTED 에 넣으면 LLM 노출 릴레이 목록에서
#   제외되어 LLM 이 수온히터·배수밸브 상호배타 룰을 판단할 데이터가 사라짐.
_PROTECTED_SEMS = frozenset({'lighting_flag', 'irrigation_flag'})


# ────────────────────────────────────────────────────────────────────
# sem 별 메타 태그 frozenset 자동 도출 헬퍼.
...
# ────────────────────────────────────────────────────────────────────
# RelayDef 생성 헬퍼 — desc/flags 자동 도출.
...
# ════════════════════════════════════════════════════════════════════
# 도메인 enum SSOT — LLM 도구·프롬프트 enum 자동 생성용.
# 신규 단계/모드 추가 시 본 파일만 수정 → 전 코드 자동 반영.
# ════════════════════════════════════════════════════════════════════
...
# ────────────────────────────────────────────────────────────────────
# CIRCULATION_MODES 표 — DB 우선 / 코드 폴백.
...
# ────────────────────────────────────────────────────────────────────
# house_id 별 릴레이 매핑 dict 반환 — 2호는 E타입,
# 그 외는 STANDARD. 모든 매핑 헬퍼의 진입점 역할.
# house_id 는 string('2') / int(2) 양쪽으로 들어오므로 반드시 int 정규화 후 비교
#   (string 비교는 항상 False → 2호에 STANDARD 가 잘못 적용되어 라벨이 뒤섞임).
# ────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────
# DB 컬럼명(relay_Nst_flag) → RelayDef 양방향 조회.
# house_id 미지정 시 STANDARD 우선, E 폴백. 없으면 None.
# ────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────
# STANDARD 시멘틱 영문기능명 set — conversion.py 등에서
# DB row 키 매칭용으로 사용.
# ────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────
# STANDARD + E 머지 헬퍼 — control_common 의 SEMANTIC_LABELS /
...
# ────────────────────────────────────────────────────────────────────
# 한글기능명/별칭 → sem dict — STANDARD/E 매핑의 kor_func +
...
# ────────────────────────────────────────────────────────────────────
# flags 에 'PROTECTED' 가 설정된 시멘틱 키 set 자동 도출.
...
# ────────────────────────────────────────────────────────────────────
# STANDARD ∪ E 의 모든 RelayDef — sem 중복은 STANDARD 우선.
...
# ────────────────────────────────────────────────────────────────────
# LLM 제어 판단용 장치 상세 설명 텍스트 자동 생성.
# 형식 (한 줄=한 장치): - <sem> — <kor_func> (<kor_relay>): <desc>
# STANDARD 우선 노출, E타입 전용 sem 만 별도 라인.
# desc 는 _RELAY_DESCRIPTIONS dict 에서 호출 시점에 직접 조회 — RelayDef.desc
# 캐시를 우회하여 _RELAY_DESCRIPTIONS 수정이 서비스 재시작 없이 즉시 반영 (SSOT).
# ────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────
# LLM 시스템 프롬프트용 장치명 매핑 한 줄 요약 텍스트.
