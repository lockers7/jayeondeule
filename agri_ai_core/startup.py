# ════════════════════════════════════════════════════════════════════
# 애플리케이션 초기화 모듈 — DB·ChromaDB·Ollama·스케줄러를 단계별 초기화
# 하고 종료 시 역순으로 정리. FastAPI lifespan 과 백그라운드 데몬에서 공통 사용.
# --->
# initialize_app : 4단계 초기화 (Ollama → ChromaDB → PostgreSQL → 스케줄러)
# shutdown_app   : 4단계 정리 (AI 루프 → 스케줄러 → ChromaDB → 기타)
# ════════════════════════════════════════════════════════════════════
import os
import sys
import time
...
# ────────────────────────────────────────────────────────────────────
# 애플리케이션 시작 초기화 — 의존성 주입 후 4단계 부트스트랩.
# 중복 호출은 _initialized 플래그로 차단(Streamlit 리렌더링 안전).
...
# ────────────────────────────────────────────────────────────────────
# 애플리케이션 종료 처리 — initialize_app 의 역순으로 4단계 정리.
# 1) AI 순환 제어 루프 정지
# 2) 스케줄러 중지
# 3) ChromaDB 연결 정리 (heartbeat 로 상태 확인)
# 4) 기타 리소스 정리
# 각 단계 실패는 경고 로그만 남기고 다음 단계 진행 (best-effort).
# ────────────────────────────────────────────────────────────────────
def shutdown_app():
    try:
        logger.info("=" * 60)
