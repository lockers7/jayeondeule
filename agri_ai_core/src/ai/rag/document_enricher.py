# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# LLM 문서 이해/요약 모듈
# 문서를 LLM에 전달하여 요약과 핵심 QA 쌍을 생성하고
# 임베딩하여 farm_knowledge에 저장합니다.
# --->
# _call_llm: Ollama /api/generate 호출 공통 함수. 응답 텍스트 반환, 실패 시 빈 문자열.
# _generate_summary: 문서 요약 생성 (LLM 호출 1회). 실패 시 빈 문자열 반환.
# _parse_qa_response: LLM 응답에서 QA JSON 배열을 파싱한다.
# _generate_qa_pairs: 핵심 QA 쌍 생성 (LLM 호출 1회). 실패 시 빈 리스트 반환.
# _store_enriched_data: 요약 및 QA 데이터를 임베딩하여 farm_knowledge에 저장한다.
# enrich_document: 문서 LLM 요약 + QA 쌍 생성 메인 진입점
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
import os
import re
import json
...
# - 핵심 QA 3~5쌍 생성 (LLM 호출 1회)
# - 요약/QA를 임베딩하여 farm_knowledge에 저장
# - 실패 시에도 기존 파이프라인에 영향 없음
# Returns: dict: {success, summary_stored, qa_pairs_generated, error}
# ============================================================
def enrich_document(
