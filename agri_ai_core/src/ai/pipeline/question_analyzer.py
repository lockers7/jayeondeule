# ------------------------------------------------------------------------------------------------------------
# 1단계: 질문유형분석 모듈
# LLM이 사용자 질문을 분석하여 데이터 수집 계획(JSON)을 생성
# 키워드 기반 처리를 최소화하고, LLM이 직접 판단
# - fast_classify(): 극히 명확한 인사만 규칙으로 분류 (LLM 호출 절약)
# - analyze_question(): LLM 기반 분석 (핵심)
# - _parse_analysis_json(): LLM 응답에서 JSON 추출
# ------------------------------------------------------------------------------------------------------------
import json
import re
import time
