# ------------------------------------------------------------------------------------------------------------
# 3단계 파이프라인 전용 프롬프트
# 모든 단계에서 LLM이 판단하며, 단계가 진행될수록 정교해지는 프로그레시브 구조
#
# [1단계] ANALYZER_SYSTEM_PROMPT — 질문유형분석 + 데이터 수집 계획
# [2단계] DATA_VALIDATOR_PROMPT — 수집 데이터 충분성 판단 + 보충 수집 계획
# [3단계] build_answer_system_prompt() — 수집된 데이터 기반 정교한 답변 + 출처 선별
#
# 일관성 원칙:
# - 1단계에서 분석한 intent/question_type이 2단계, 3단계까지 전달
# - 뒤로 갈수록 더 많은 데이터와 맥락을 바탕으로 정교한 판단
# - 키워드 기반 판단 없음. 모든 내용 판단은 LLM이 수행
# ------------------------------------------------------------------------------------------------------------

# ============================================================
# [1단계] 질문유형분석 프롬프트
# 역할: 질문을 이해하고, 정확한 답변을 위해 어떤 데이터를 수집해야 하는지 계획
# LLM이 질문의 의도, 지역명, 장치명, 파일명 등을 모두 직접 판단
# ============================================================
...
# ============================================================
# [2단계] 데이터 충분성 판단 프롬프트
# 역할: 1단계의 질문 의도와 수집된 데이터를 비교하여
#       답변하기에 충분한지 판단하고, 부족하면 보충 수집 계획을 생성
# 1단계의 intent/question_type을 그대로 이어받아 일관성 유지
# ============================================================
...
# ============================================================
# [3단계] 답변작성 프롬프트
# 역할: 수집된 데이터 기반으로 정교한 답변 생성 + 사용한 출처 선별
# 1단계의 분석(intent, question_type)이 여기까지 전달되어 답변 방향을 통제
# ============================================================
