...
# [3단계] 답변작성 프롬프트
# ═════════════════════════
def _build_general_prompt(speech_style: str) -> str:
    """general 유형(단위변환·계산·상식·정의·번역·농업 이론 등) 전용 초간결 프롬프트.

    외부 데이터 없이 LLM 자체 지식으로 답변하는 질문이므로 농장정보/히스토리 규칙을 모두 제거한다.
    시스템 프롬프트 2745→약 350토큰으로 축소하여 prompt_eval 시간 대폭 단축.
    """
    ...
    return (
        ...
        f"{tone}\n\n"
        "**답변 규칙:**\n"
        "1. 질문이 단위 변환·계산·상식·정의·번역·농업 이론 등이라면 당신의 지식만으로 즉시 답변하세요.\n"
        "2. 계산·변환은 결과 수치를 명확히 명시하세요 (예: 1650mm = 1.65m).\n"
        "3. 확신이 없는 사실은 지어내지 마세요.\n"
        "4. 답변은 질문 난이도에 맞게 간결하게. 불필요한 배경 설명·예시·표는 넣지 마세요.\n"
        "5. 내부 추론/think 태그 절대 미출력.\n"
        "/no_think"
    )
...
def build_answer_system_prompt(farm_name, farm_info, speech_style="male",
    ...
    if qtype_early == "general":
        return _build_general_prompt(speech_style or "male")
