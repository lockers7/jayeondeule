...
def db_describe_table(table_name: str) -> Dict[str, Any]:
    try:
        ...
        # GAP1/2: 테이블 조사 성공 시 실제 컬럼을 지식으로 영속 학습(조사=영구 기억).
        try:
            import threading
            threading.Thread(target=_autolearn_schema_worker,
                             args=([str(table_name).lower()],), daemon=True).start()
        except Exception:
            pass
...
# ────────────────────────────────────────────────────────────────────
# 자가교정 → 지식 영속 학습 (GAP2). 컬럼/타입 오류로 실제 스키마를 알아냈으면
# system_knowledge 에 비동기 저장 → 같은 컬럼 환각의 영구 재발을 차단한다.
# best-effort · 데몬 스레드(조회 응답을 임베딩 지연으로 막지 않음).
# ────────────────────────────────────────────────────────────────────
def _autolearn_schema_worker(tables):
    try:
        ...
        for t in tables:
            ...
            if not cols:
                continue
            text = (f"[DB테이블 {t}] 실제 컬럼(자가학습): {', '.join(cols)}. "
                    f"이 테이블 조회 시 이 컬럼만 사용 — 존재하지 않는 컬럼 환각 금지.")
            save_system_knowledge(text, category="db_schema",
                                  source="auto_selfheal", key=f"cols_{t}")
    except Exception:
        pass


def _autolearn_schema_from_error(q, exc):
    try:
        code = getattr(exc, "pgcode", None)
        if code not in ("42703", "42883", "22P02", "42804", "42P18"):
            return
        tables = sorted({t.lower() for t in _TABLE_REF.findall(q)})
        if not tables:
            return
        import threading
        threading.Thread(target=_autolearn_schema_worker,
                         args=(tables,), daemon=True).start()
    except Exception:
        pass
...
def db_read_query(sql: str, limit: int = 50,
    ...
    try:
        ...
        try:
            ...
            _autolearn_schema_from_error(q, ex)  # GAP2: 실제 스키마를 지식으로 영속(재발 차단)
            # 자가교정 힌트 제공 여부를 로그에 명시 → 모니터링이 self-heal 발생을 관찰 가능.
            logger.warning(
                f"[DB조회도구] 실패: {ex}"
                f"{' [자가교정:스키마힌트제공]' if hint else ''} — {q[:120]}")
            err = str(ex)
            out = {"success": False, "error": (f"{err}\n💡 {hint}" if hint else err)}
            if hint:
                out["hint"] = hint       # 구조화 필드 — LLM/도구가 정확한 컬럼으로 즉시 재시도
                out["self_heal"] = True  # 자가교정 힌트 제공됨(재시도 유도 + 모니터링 신호)
            return out
        finally:
            db._putconn(conn)
    except Exception as e:
