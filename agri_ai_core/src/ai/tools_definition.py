...
# get_system_prompt_with_tools: 시스템 프롬프트 생성 (답변 범위 제한 + 데이터 정확성 + 검색어 한국어 필수 + 대화 연속성 맥락 활용 가이드 포함)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
5. 인사/감정/의견/일상대화/이전대화 요약 요청: 도구 없이 응답 가능합니다. 사용자가 이전 대화 내용을 물으면 시스템이 제공한 [직전 대화 맥락]과 [관련 과거 대화 주제]를 참고하여 답변하세요.
