...
# --->
# get_available_tools: 사용 가능한 도구 목록 (search_web query에 구체적 의도 반영 가이드 포함)
# get_system_prompt_with_tools: 시스템 프롬프트 생성 (답변 범위 제한 + 데이터 정확성 + 검색어 구체성 가이드 포함)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "query": {
                            "type": "string",
                            "description": "검색할 키워드. 사용자 질문의 핵심 의도를 구체적으로 포함하세요. 예: '정읍 날씨' → '정읍 오늘 날씨 기온'"
                        },
                        "n_results": {
                            "type": "integer",
...
- `search_web` query에 질문 핵심 의도를 구체적으로 포함하세요. 예: "정읍 날씨"→"정읍 오늘 날씨 기온". 지명만 단독 검색 금지.
