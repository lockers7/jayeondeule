...
# --->
# get_available_tools: 사용 가능한 도구 목록 (search_web query에 한국어 필수 + 구체적 의도 반영 가이드 포함)
# get_system_prompt_with_tools: 시스템 프롬프트 생성 (답변 범위 제한 + 데이터 정확성 + 검색어 한국어 필수 + 과거 대화 분리 가이드 포함)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    return [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "농장의 실시간 센서+릴레이 데이터를 가져옵니다. data_type='all'(기본값)로 센서+릴레이를 한 번에 조회하세요. sensor나 relay 단독 조회는 필요시에만 사용합니다.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "farm_id": {
...
**과거 대화 처리 (절대 규칙):**
- 시스템이 제공하는 "세션 컨텍스트"는 대화 연속성 파악용일 뿐, 답변 근거로 사용 금지.
- 현재 질문과 도구 결과만으로 답변. 과거 대화의 수치/답변 내용을 현재에 재사용 금지.
- 이전 질문과 유사해도 반드시 도구를 새로 호출하여 최신 데이터 확인.
