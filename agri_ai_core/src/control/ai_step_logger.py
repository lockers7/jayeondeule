# ══════════════════════════════════════════════════════════════════════════════
# 단계별 로그 포맷터 (M3, 통합)
# 알고리즘 모드의 "[1/N] 농장 X, 재배사 Y" 와 동일한 형태로
...
class AiStepLogger:
    ...
    # ────────────────────────────────────────────────────────────────
    def _head(self) -> str:
        # step prefix 가 메인 — "[AI 1/14] 0-99: 라벨" 형식.
        # scope 가 비어 있으면 "[AI 1/14]: 라벨".
        if self.scope:
    ...
    # ────────────────────────────────────────────────────────────────
    # 직전 step()/skip()/warn() 본문을 여러 줄로 자세히 출력.
    ...
    def detail(self, *lines, max_chars: int = 800) -> None:
        # step 헤더와 일관된 prefix 로 detail 라인도 [{prefix} N/M] 시작.
