# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# AI 릴레이 제어 모듈
# LLM이 센서값·릴레이 상태·생육단계·최적조건을 종합 분석하여 릴레이 값을 결정
# 선행 조치: 센서 트렌드 분석으로 임계치 도달 전 예방 제어
# --->
# _log_ai_status: 상세 로깅 함수 (공통 포맷팅 함수 활용)
# _log_ai_decision: AI 판단 결과 상세 로그
# _detect_trend: 트렌드 분석 (선행 조치 핵심)
# _check_threshold_proximity: 임계치 근접 판단
# monitor_ai_emergency: 10초 주기 AI 센서 모니터링 (긴급 개입 판단)
# _build_system_prompt: 시스템 프롬프트
# _build_user_prompt: 유저 프롬프트
# _call_llm: LLM 호출
# _parse_relay_response: JSON 응답 파싱
# _validate_safety: 안전 검증
# control_ai_environment: 단일 재배사 AI 환경제어 (LLM 정기 호출 / 긴급 개입)
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
import os
import re
import json
